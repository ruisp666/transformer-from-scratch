...
import torch
from scaled_attention import scaled_dot_attention
...
class MultiHeadAttention(nn.Module):
    ...
    def combine_heads(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # could use .transpose(2,1)
        return x.permute(0,2,1,3).reshape(x.shape[0], -1, self.d_model )
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
        ...
        multihead_attention = self.combine_heads(scaled_dot_attention(Q,K,V))
